...
*   **Cover Photo Support**: Automatically displays cover photos from Notion pages, preserving the look and feel of your original Notion content.
...
*   **Breadcrumb Navigation**: Provides navigation breadcrumbs showing the path from the main page to the current subpage.
...
*   **Table Header Support**: Detailed handling of both column and row headers in tables.
...
*   **SEO & Robot Control**:
    ...
    *   Includes a `robots.txt` file to block standard crawlers and specific AI crawlers (`Google-Extended`, `GPTBot`, `ChatGPT-User`, `CCBot`, `ClaudeBot`, `anthropic-ai`, etc.) from accessing the site.
...
*   **Basic Error Handling**: Includes a custom styled `error.php` page for 404 and 500 errors.
*   **Performance Optimizations**: 
    *   Implements Gzip compression for faster page loads.
    *   Configures browser caching rules for static resources.
...
*   **Numbered Lists**: Some inconsistencies may exist in the rendering of numbered lists compared to Notion's original formatting.